...
- `index.html`은 브라우저의 이전 자산 재사용을 막기 위해 `app.js?v=1.2.19`, `styles.css?v=1.2.19` 캐시 키를 사용한다. 실행 파일 변경 시 두 버전을 함께 올린다.
...
- 전하 획득은 **반드시 적(더미) 명중 시에만** 일어난다(빗맞거나 단순 시전으로는 안 쌓임): 일반 Q(뇌격) 명중 **+2**(`addCharge(2,"Q")`), W 2타(방출)는 **반경 내 명중 시 +1**(`hits.length>0`일 때만), E1 백스텝·E2 볼트 러시 명중 각 **+1**, R1·R2 낙뢰 명중 각 **+1**. **W 1타(충전 윈드업)와 D(반격)는 전하를 주지 않는다**.
