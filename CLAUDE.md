...
- **D(빗겨 흘리기) 시전 중에는 R·E 포함 모든 스킬 불가**(`if (isCasting("D") && key !== "D") return false`). `CAST.D=0.75`초 동안 제자리에 고정된 뒤 `RANGE.D_DASH=3m`를 돌진하며 경로 피해를 준다. 추가로 D 중 **R2·E2는 버퍼(선입력)도 안 됨**(`requestSkill`에서 D 시전 중 `isRRecast`/`isERecast` 입력은 버리고 버퍼 안 함). D는 무적/방어 연출(`.parry`, `spawnParry`) 없이 기본 적용되며, D 경로 피해가 1명 이상 적중하면 전하를 +1 획득한다.
...
- **R**: 1차 낙뢰는 **시전 시작(키 누른) 시점 커서**에 지정(`rTarget`, `R_CAST` 클램프) → 2차는 **시전시간 없이 즉시** 그 위치로 순간이동(`teleportTo`). R1 적중 시 전하 +1. R2 텔레포트 직후 `pendingE1Release`가 있으면 저장된 E 목표 좌표를 향해 E1을 방출한다. R2 적중 시 `castSecondLightning()` 내부에서 전하 +1을 먼저 적용하고, 일반 상태면 `state.pendingR2FullCharge = R2_FULL_CHARGE_DELAY(0.1s)`를 걸어 0.1초 뒤 `fillCharge("전하 최대")`로 풀충전한다. 이때 즉시 `enterOvercharge(..., force)`하지 않고 기존 전하 5스택 전환 기준(`updateTimers`의 `state.charge >= MAX_CHARGE && state.pendingR2FullCharge <= 0 && !enemyNearby()`)을 따른다. 이미 과전하 상태에서 수동 R2가 적중하면 `enterOvercharge("낙뢰 과전하", true, { preserveW: true })`로 탄환·지속시간을 재충전하고 백스텝(E)을 초기화하지만, 진행 중인 W 차징·차징 바·W1 사운드·음성 페어는 유지한다. 미사용 시 `updateTimers`가 자동 2타(`castSecondLightning`). **R 위빙 콤보**: R1 시전 중 또는 R2 준비 후 E→R 입력 모두 R2 위치에서 E1 발동.
...
- 에이든 발밑 `#charStacks`(`renderUnits`에서 매 프레임 위치/내용 갱신)에 현재 전하/탄환 스택을 표시. 빗겨 흘리기(D)는 흰색 아우라 무적/방어 연출 없이 시전 시간 뒤 돌진 피해만 표시한다.
...
- 데미지는 `damageDummy` 단일 경로. 더미는 **죽지 않고 체력이 최소 1로 고정**(`clamp(hp-amount, 1, 100)`)되며, 사망/부활 로직은 없다(그래서 표식이 사망으로 풀리지 않음). 피격 시 `dummy.regenDelay=1.2`가 찍히고, `updateDummies`가 그 지연 후 **초당 `DUMMY_REGEN`(=12)으로 체력을 자연 회복**(최대 100)시킨다(회복은 더미 이동 토글과 무관하게 매 프레임). `#dummyMoveMode` 하나로 더미 이동을 제어한다. 켜는 순간 각 더미의 현재 위치를 `moveAnchorX/Y`로 저장하고 중심 기준 좌우 3m(`DUMMY_MOVE_RANGE`)를 `moveSpeedMps() * M` 속도로 등속 왕복한다. 기존 랜덤 바운스 이동과 별도 `#dummyStrafeMode`는 제거했다.
...
- `enterOvercharge()`는 호출 전 `!isOvercharged()`를 `changedFromMelee`로 고정하고, 근접→원거리 전환일 때만 `playVoice("passive")`를 호출한다. 과전하 중 force 재진입은 음성을 요청하지 않는다. 세 번째 옵션 `preserveW`는 과전하 중 수동 R2 적중 재진입 경로에서 W 상태 초기화를 건너뛴다. 모든 캐릭터 대사는 `activeVoice` 단일 채널을 공유하며, 현재 대사가 끝나기 전 신규 요청은 중단·큐잉 없이 폐기한다.
- W 차징 바는 `state.wChargeHudActive`로 캐스트·이동 상태와 분리한다. W1 시작 시 활성화하고 W2 방출·일반 과전하 취소·초기화에서 비활성화한다. 과전하 중 수동 R2 적중 재진입은 `preserveW`로 유지한다.
...
### 추가 효과 1.2
`#extraEffectsBtn`이 `#extraEffectModal`을 열고 4개 토글(`effectWillpower`, `effectGlamour`, `effectThunder`, `effectAwakening`)을 제공한다. 충전 - 섬광과 캐릭터 레벨 입력은 제거했다. 각 토글은 `.extra-effect-option`이며 hover/focus 시 바로 아래 `.extra-effect-desc` 설명을 표시한다. 진행 상태는 `state.extraEffects`에 저장하지만 모달 하단 상태 패널은 제거했다. `resetChampion()`과 `reset()`은 추가 효과 진행도와 쿨다운을 초기화한다. 쿨타임 초기화 모드가 켜져 있으면 `resetExtraEffectCooldowns()`와 `setExtraEffectCooldown()` 경유로 아이템 쿨다운도 0초가 된다. 효과 중첩은 허용하지 않는다(의념 단일 충전, 각성 active 중 재발동 차단, 뇌명 집행 active 중 재발동 차단). 현란함은 고유 중첩 효과라 최대 8중첩을 허용한다. 쿨다운 초기화 토글 변경과 매 프레임 `updateExtraEffects()`가 아이템 쿨다운 상태를 실시간 반영한다. 아이템 발동 시 `spawnItemEffect(point, kind, size)`가 `.fx-item-*` 링 이펙트를 생성하고 `logEvent(..., "item")`으로 금색 로그를 남긴다.

- 의념: 스킬 사용 시 `willpower.armed=3`, 쿨다운 2초. 다음 기본 공격 적중 시 근거리 대상 최대 체력 12%, 원거리 11% 추가 피해.
- 뇌명 집행: `damageDummy`의 기본 공격/스킬 피해 2회가 5초 안에 누적되면 8초 동안 방어 관통 +12 상태, 쿨다운 15초. 더미 HP가 1로 내려가는 순간을 처치 관여 대체 판정으로 삼아 크레딧 +25, 이동 속도 +20% 2초, 상태 제거.
- 각성: R1 사용 시 8초 동안 공격 속도 +30%, 이동 속도 +12%, 쿨다운 20초. 현재 충돌 시스템이 없어 충돌 무시는 상태 기록만 한다.
- 현란함: 피해 적중마다 3초 중첩 추가, 최대 8중첩. 근거리 중첩 +3%, 원거리 중첩 +1.5% 이동 속도.

## 주의사항

- 입력→상태→렌더가 단방향이라, 즉시 반영이 필요한 핸들러는 명시적으로 `render()`를 호출한다(버튼 클릭 등). 루프가 곧 다시 그리므로 대부분은 생략 가능.
- 옵션 모달 토글(`els.cooldownResetMode`, `cameraShakeMode`, `alexMode` 등)은 DOM 체크박스를 직접 진실의 원천으로 읽는다 — 별도 상태 변수 없음. "미안해.. 알렉스"(`#alexMode`)를 켜면 `renderUnits`가 더미를 `Alex_Mini_00.png`(`.alex-sprite`)로 그린다(`.dummy-core` 대체). "초기화" 버튼은 제거됨(`reset()`은 로드 시 1회만 호출).
